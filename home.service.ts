...
export class HomeService {
    ...
    getApps() {
        return this.http.get('http://localhost:4200/application')
    ...
    addApplication(app: Application) {
        ...
        return this.http.post('http://localhost:4200/application', body, { headers: headers })
            .map((res: Response) => {
    ...
    updateApplication(application: Application) {
        ...
        return this.http.patch('http://localhost:4200/application/' + application.appid, body, {headers: headers})
    ...
    deleteApplication(app: Application) {
        ...
        const headers = new Headers({ 'Content-Type': 'application/json', 'Accept': 'application/json' });
        return this.http.delete('http://localhost:4200/application/' + app.appid, { headers: headers })
            .map((response: Response) => response.json());
        //.catch((error: Response) => Observable.throw(error.json()));
    }

    getDashboard(column: String) {
        return this.http.get('http://localhost:4200/application/dash/' + column)
            .map((response: Response) => {
                const apps = response.json().obj;
